...
`onUpdate` is a [hook](https://www.reatom.dev/packages/hooks) which subscribes to the atom and calls passed callback on every update.
...
- Currently, there is no way to subscribe on error of any dependency, but we are working on it. In [reatomAsync](https://www.reatom.dev/packages/async) passed effects wraps to an error handler and allow you to handle errors, but again - you should wrap it explicit.
