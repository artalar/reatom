...
- **combobox** — Korean IME + `autoSelect` focus between composition steps
  ...
  (0.3.0 — our `comboboxValue.ts` normalizes NFD for *matching*, check the
  *completion* path); Ctrl/Cmd non-paste shortcuts on items without virtual
...
- **radio** — tabbing back into a group focuses the *checked* radio after an
...
- **View-only components** — `button`, `role`, `group`, `heading` /
  ...
  model logic.
- **Anchor *components*** (`PopoverAnchor`, `MenuAnchor`, `SelectAnchor`,
