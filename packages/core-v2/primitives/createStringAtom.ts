...
let count = 0
export function createStringAtom(initState?: string, options?: AtomOptions): StringAtom
export function createStringAtom<T extends string>(initState: T, options?: AtomOptions): StringAtom<T>
export function createStringAtom(initState = ``, options: AtomOptions = `string${++count}`): StringAtom {
  // @ts-ignore
