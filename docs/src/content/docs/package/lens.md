...
Sometimes you need to get computed value from an atom in another computer, but you don't want to trigger other recomputations if the computed value is not changed. It is the common case for `reatomComponent` from [reatom/npm-react](/package/npm-react/) package. The `select` allows you to perform and memorize some computation by a simple inline callback. It is specially useful when you can't create separate memorized function because your target atom is dynamically created.
