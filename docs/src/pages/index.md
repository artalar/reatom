...
- perfect **effects management** and **debugging** experience by [immutable cause (call) stack](/guides/debug), advanced [async package](/packages/async) allows you to describe complex async flows with caching, retrying and automatic cancellation of entire query sequences - like redux-saga, but with native `await` and `AbortController`
