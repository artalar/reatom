...
;[10, 100].forEach(count =>
  [1, 10, 50].forEach(subscribersCount => {
    ;['ALL', 'COMPLETED', 'ACTIVE'].forEach((filter: any) => {
      suite.add(
        `(${subscribersCount}) ${count} todos | ${filter} [reatom]`,
        () => {
          const store = reatomStore

          store.dispatch(RA.setVisibilityFilter(filter))

          times(subscribersCount, () =>
            store.subscribe(RA.TodosContent, () => {}),
          )

          times(count, i =>
            store.dispatch(RA.addTodo({ id: i, text: `MyTodo ${i}` })),
          )
      ...
      suite.add(
        `(${subscribersCount}) ${count} todos | ${filter} [redux]`,
        () => {
          ...
          times(count, i =>
            store.dispatch(RE.addTodo({ id: i, text: `MyTodo ${i}` })),
          )
